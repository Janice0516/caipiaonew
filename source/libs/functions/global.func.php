...
function money_unit_factor($unit) {
	if ($unit == 'jiao') return 0.1;
	if ($unit == 'fen') return 0.01;
	if ($unit == 'li') return 0.001;
	return 1;
}

function money_unit_label($unit) {
	if ($unit == 'jiao') return '角';
	if ($unit == 'fen') return '分';
	if ($unit == 'li') return '厘';
	return '元';
}

function money_to_yuan($amount, $unit) {
	return round(floatval($amount) * money_unit_factor($unit), 3);
}

function money_from_yuan($amount, $unit) {
	$factor = money_unit_factor($unit);
	if ($factor == 0) return 0;
	return round(floatval($amount) / $factor, 3);
}
