		...
		<table width="100%" cellspacing="0">
			<thead>
				<tr>
					...
					<th align="center" width="80">自动开奖</th>
			...
			<?php foreach ($list as $v){?>
				<tr id="list_<?php echo $v['id']?>">
					...
					<td align="center">
					    <a href="javascript:;" onclick="toggle_auto_draw(<?php echo $v['id']?>, <?php echo $v['auto_draw']?>);" id="auto_draw_btn_<?php echo $v['id']?>">
					        <?php echo $v['auto_draw'] == 1 ? '<span style="color:green;font-weight:bold;">开启</span>' : '<span style="color:red;font-weight:bold;">关闭</span>'; ?>
					    </a>
					</td>
					<td align="center">
...
function toggle_auto_draw(id, current_state) {
    var new_state = current_state == 1 ? 0 : 1;
    var action_str = new_state == 1 ? "开启" : "关闭";
    if (confirm("确定要【" + action_str + "】此彩种的自动发号开奖吗？")) {
        $.post("<?php echo ADMIN_PATH?>&c=game&a=toggle_auto_draw", {id: id, auto_draw: new_state}, function(data) {
            if (data.status == 1) {
                location.reload();
            } else {
                alert(data.info);
