			...
			if ($r) {
				if ($r['account'] == 0 && $r['tui'] == 0) {//注单未结算
					if ($this->db->update(array('tui' => 1), array('id' => $id))) {
						...
						$countmoney = round(floatval($user['money']) + floatval($r['money']), 3);
					...
					if ($this->db->update(array('tui' => 2), array('id' => $id))) {
						...
						if ($r['account'] > 0) {//赢了退回
							$account = '-='.$r['account'];
							$countmoney = round(floatval($countmoney) - floatval($r['account']), 3);
		...
		foreach($idarr as $id) {
			...
			if ($r) {
				if ($this->db->update(array('tui' => 3), array('id' => $id))) {
					...
					if ($r['account'] > 0) {//赢了退回
						$account = '-='.$r['account'];
						$countmoney = round(floatval($user['money']) - floatval($r['account']), 3);
						$db2->update(array('money' => $account), array('uid' => $r['uid']));//盈利资金退还
						//流水记录
