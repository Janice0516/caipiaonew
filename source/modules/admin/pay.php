...
class pay extends admin{
	...
	public function addto() {
		...
		if ($r) {
			...
			$uid = $r['uid'];
			$rate = isset($dict[$r['type2']]) ? $dict[$r['type2']] : 1;
			$money = round(floatval($r['money']) * floatval($rate), 3);
			$user = $this->db2->get_one(array('uid' => $uid));
			if (!$user) {
				...
				$gift_money_from_rule = function($recharge_money, $gift_rule, $cap) {
					$gift_rule = floatval($gift_rule);
					if ($gift_rule <= 0) {
						return 0;
					}
					$gift_money = $gift_rule < 1 ? floatval($recharge_money) * $gift_rule : $gift_rule;
					$gift_money = round(floatval($gift_money), 3);
					$cap = floatval($cap);
					if ($cap > 0 && $gift_money > $cap) {
						$gift_money = round($cap, 3);
					}
					return $gift_money > 0 ? $gift_money : 0;
				};
				...
				$first_money = 0;
				if(count($first) == 0){
					if(!empty($gift['firstRechargeMoney5']) && $money >= floatval($gift['firstRechargeMoney5'])){
						$first_money = $gift_money_from_rule($money, $gift['firstRechargeMoneyGive5'], 0);
					}else if(!empty($gift['firstRechargeMoney4']) && $money >= floatval($gift['firstRechargeMoney4'])){
						$first_money = $gift_money_from_rule($money, $gift['firstRechargeMoneyGive4'], 0);
					}else if(!empty($gift['firstRechargeMoney3']) && $money >= floatval($gift['firstRechargeMoney3'])){
						$first_money = $gift_money_from_rule($money, $gift['firstRechargeMoneyGive3'], 0);
					}else if(!empty($gift['firstRechargeMoney2']) && $money >= floatval($gift['firstRechargeMoney2'])){
						$first_money = $gift_money_from_rule($money, $gift['firstRechargeMoneyGive2'], 0);
					}else if(!empty($gift['firstRechargeMoney']) && $money >= floatval($gift['firstRechargeMoney'])){
						$first_money = $gift_money_from_rule($money, $gift['firstRechargeMoneyGive'], 0);
					}
				}
				//每日首冲送金额
				...
				$day_first_money = 0;
				if(count($day_first) == 0){
					$day_first_cap = isset($gift['dayFirstRechargeMax']) ? $gift['dayFirstRechargeMax'] : 0;
					if(!empty($gift['dayFirstRechargeMoneyGive5']) && $money >= floatval($gift['dayFirstRechargeMoney5'])){
						$day_first_money = $gift_money_from_rule($money, $gift['dayFirstRechargeMoneyGive5'], $day_first_cap);
					}else if(!empty($gift['dayFirstRechargeMoneyGive4']) && $money >= floatval($gift['dayFirstRechargeMoney4'])){
						$day_first_money = $gift_money_from_rule($money, $gift['dayFirstRechargeMoneyGive4'], $day_first_cap);
					}else if(!empty($gift['dayFirstRechargeMoneyGive3']) && $money >= floatval($gift['dayFirstRechargeMoney3'])){
						$day_first_money = $gift_money_from_rule($money, $gift['dayFirstRechargeMoneyGive3'], $day_first_cap);
					}else if(!empty($gift['dayFirstRechargeMoneyGive2']) && $money >= floatval($gift['dayFirstRechargeMoney2'])){
						$day_first_money = $gift_money_from_rule($money, $gift['dayFirstRechargeMoneyGive2'], $day_first_cap);
					}else if(!empty($gift['dayFirstRechargeMoneyGive']) && $money >= floatval($gift['dayFirstRechargeMoney'])){
						$day_first_money = $gift_money_from_rule($money, $gift['dayFirstRechargeMoneyGive'], $day_first_cap);
					}
				}
				//每次充值送金额
				$boutfirst_money = 0;
				$recharge_cap = isset($gift['rechargeGiveMax']) ? $gift['rechargeGiveMax'] : 0;
				if(!empty($gift['rechargeMoneyGive5']) && $money >= floatval($gift['rechargeMoney5'])){
					$boutfirst_money = $gift_money_from_rule($money, $gift['rechargeMoneyGive5'], $recharge_cap);
				}else if(!empty($gift['rechargeMoneyGive4']) && $money >= floatval($gift['rechargeMoney4'])){
					$boutfirst_money = $gift_money_from_rule($money, $gift['rechargeMoneyGive4'], $recharge_cap);
				}else if(!empty($gift['rechargeMoneyGive3']) && $money >= floatval($gift['rechargeMoney3'])){
					$boutfirst_money = $gift_money_from_rule($money, $gift['rechargeMoneyGive3'], $recharge_cap);
				}else if(!empty($gift['rechargeMoneyGive2']) && $money >= floatval($gift['rechargeMoney2'])){
					$boutfirst_money = $gift_money_from_rule($money, $gift['rechargeMoneyGive2'], $recharge_cap);
				}else if(!empty($gift['rechargeMoneyGive']) && $money >= floatval($gift['rechargeMoney'])){
					$boutfirst_money = $gift_money_from_rule($money, $gift['rechargeMoneyGive'], $recharge_cap);
				}

				$money = round(floatval($money), 3);
				$first_money = round(floatval($first_money), 3);
				$day_first_money = round(floatval($day_first_money), 3);
				$boutfirst_money = round(floatval($boutfirst_money), 3);
				$add_money = round(floatval($money) + floatval($first_money) + floatval($day_first_money) + floatval($boutfirst_money), 3);
				if ($this->db2->update(array('money' => '+='.$add_money), array('uid' => $uid))) {//充值
					...
					if($first_money > 0){
						$countmoney = round(floatval($countmoney) + floatval($first_money), 3);
					...
					if($day_first_money > 0){
						$countmoney = round(floatval($countmoney) + floatval($day_first_money), 3);
						$this->db3->insert(array('uid'=>$uid, 'money'=>$day_first_money, 'countmoney'=>$countmoney, 'type'=>5, 'addtime'=>SYS_TIME, 'comment'=>'每日首冲送金额'));
					}

					if($boutfirst_money > 0){
						$countmoney = round(floatval($countmoney) + floatval($boutfirst_money), 3);
