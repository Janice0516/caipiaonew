...
class settings extends admin {
	...
	public function init() {
		if (isset($_POST['dosubmit'])) {
			...
			if ($_FILES['firstRechargead']['size']||$_FILES['dayFirstRechargead']['size']||$_FILES['rechargead']['size']||$_FILES['wxfile']['size'] || $_FILES['alifile']['size']|| $_FILES['lunfantu1']['size']|| $_FILES['lunfantu5']['size']|| $_FILES['lunfantu2']['size']|| $_FILES['lunfantu3']['size']|| $_FILES['lunfantu4']['size']) {//如果选择了上传图片
				...
				if ($_FILES['wxfile']['size']) {//微信
					...
					if ($_FILES['firstRechargead']['size']) {//微信
					$up->filename = 'firstRechargead';
					$frreturn = $up->up();
					if ($frreturn['state'] == 'success') {
						@unlink('./uppic/ewm/'.$config['firstRechargead']);//删除原来的图像
						$setting_arr['firstRechargead'] = $frreturn['info'];
					}
				}
					if ($_FILES['dayFirstRechargead']['size']) {//微信
					$up->filename = 'dayFirstRechargead';
					$dfrreturn = $up->up();
					if ($dfrreturn['state'] == 'success') {
						@unlink('./uppic/ewm/'.$config['dayFirstRechargead']);//删除原来的图像
						$setting_arr['dayFirstRechargead'] = $dfrreturn['info'];
					}
				}
					if ($_FILES['rechargead']['size']) {//微信
					$up->filename = 'rechargead';
					$rreturn = $up->up();
					if ($rreturn['state'] == 'success') {
						@unlink('./uppic/ewm/'.$config['rechargead']);//删除原来的图像
						$setting_arr['rechargead'] = $rreturn['info'];
					}
				}
			...
			if(empty($setting_arr['firstRechargead'])) $setting_arr['firstRechargead'] = $setting_arr['firstRechargead_old'];
			if(empty($setting_arr['dayFirstRechargead'])) $setting_arr['dayFirstRechargead'] = $setting_arr['dayFirstRechargead_old'];
			if(empty($setting_arr['rechargead'])) $setting_arr['rechargead'] = $setting_arr['rechargead_old'];
