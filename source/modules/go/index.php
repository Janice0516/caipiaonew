...
class index extends go {
	...
	public function __construct() {
		// Disable HTML error output for API robustness
	...
	public function game() {//游戏首页
		...
		if ($gameid) {
			if (!isset($gamelist[$gameid])) {
				header('Location: ?a=dating');
				exit;
			}
		} else {
			$cookie_gameid = intval(get_cookie('gameid'));
			if ($cookie_gameid && isset($gamelist[$cookie_gameid])) {
				$gameid = $cookie_gameid;
			} else {
				$gameid = !empty($gamedb) ? intval($gamedb[0]['id']) : 0;
			}
			if (!$gameid || !isset($gamelist[$gameid])) {
				header('Location: ?a=dating');
				exit;
			}
		}
		...
		if(!empty($room)){
			...
			if(empty($_GET['roomid'])){
				// Auto-select the first room (Primary Room) to bypass selection page
				$roomid = 1;
			...
			if($roomConf['minimum'] > 0){
				if($user['money'] < $roomConf['minimum'] || $user['money'] > $roomConf['maximum']){
					...
					$min_show = money_fmt(money_from_yuan($roomConf['minimum'], $unit)).$unit_label;
					$max_show = money_fmt(money_from_yuan($roomConf['maximum'], $unit)).$unit_label;
					// Redirect to Game Lobby (dating) instead of self to avoid loop
					echo '<script>alert("'._lang('金额限制').$min_show.'-'.$max_show.'");window.location.href="/?a=dating"</script>';exit;
				}
			}
		...
		if ($send_money == '10-1000000' && $this -> setting['send_money'] != '10-1000000') {
			$send_money = $this -> setting['send_money'];
		}
	...
	public function order() {//注单
		...
		if ($search && $list) {//如果是，统计 有查到数据则统计 未结算的不统计
			...
			$count['count'] = round($ying_count['account'] - $ying_count['money'], 3);//得出的去掉成本的输赢总数
			$count['ruzhang'] = round($ying_count['count'], 3);
			$count['shu'] = round($count['count'] - $count['ruzhang'], 3);
			$count['ying'] = round($count['ruzhang'] - $count['shu'], 3);
		}
	...
	public function ajax_set_unit() {
		header('Content-Type: application/json; charset=utf-8');
		$user = $this -> check_user();
		if (!$user) {
			echo json_encode(array('status' => 'n', 'login' => 'y', 'info' => _lang('您尚未登录!请登录后再操作。')));
			exit;
		}
		$unit = safe_replace(trim($_POST['unit']));
		if (!in_array($unit, array('yuan', 'jiao', 'fen', 'li'))) {
		...
		if ($this -> db -> update(array('unit' => $unit), array('uid' => $user['uid']))) {
			echo json_encode(array('status' => 'y', 'info' => _lang('保存成功!')));
		} else {
			echo json_encode(array('status' => 'n', 'info' => _lang('保存失败!')));
		}
		exit;
	}
	...
	public function pay() {// 充值
		...
		if (isset($_POST['dosubmit'])) { // 提交
			$money = round(trim($_POST['money']), 3);//金额
...
public function qqqqqq() {// 注单记录 - 游戏选择
	...
	public function youhui() {// 优惠
	 ...
	 $firstRechargetitle=$this -> setting['firstRechargetitle'];
	 	 $firstRecharge_start_time=$this -> setting['firstRecharge_start_time'];
	 	 	 $firstRecharge_end_time=$this -> setting['firstRecharge_end_time'];
	 	 	 	 $firstRechargead=$this -> setting['firstRechargead'];
	
		 $dayFirstRechargetitle=$this -> setting['dayFirstRechargetitle'];
	 	 $dayFirstRecharge_start_time=$this -> setting['dayFirstRecharge_start_time'];
	 	 	 $dayFirstRecharge_end_time=$this -> setting['dayFirstRecharge_end_time'];
	 	 	 	 $dayFirstRechargead=$this -> setting['dayFirstRechargead'];
	 	 	 	 
	 	 	 $rechargetitle=$this -> setting['rechargetitle'];
	 	 $recharge_start_time=$this -> setting['recharge_start_time'];
	 	 	 $recharge_end_time=$this -> setting['recharge_end_time'];
	 	 	 	 $rechargead=$this -> setting['rechargead'];	 	 
	 ...
	 $gift=$this ->setting['gift'];
	  	$gift = unserialize(urldecode($gift));
		include template('fuli2');
	}
		 public function fuli3() {// 福利活动1
		$headername = _lang('优惠');
		$title = $headername.' - ' . $this -> setting['webname'];
		$user = $this -> check_user(); //检查登录
		if (!$user) {
		...
		if (isset($_POST['dosubmit'])) { // 提交
			...
			if ($this -> db -> update(array('money' => '-='.$money), array('uid' => $uid))) {//用户资金减
				...
				if (str_exists($this -> setting['cash'], '%')) {//百分比收费
					$service = round($money * rtrim($this -> setting['cash'] / 100, '%'), 3);
				} else {//单笔收费
					$service = round($this -> setting['cash'], 3);
				}
				//提现记录
				//查询上级代理 判断代理属性
				$agents = 0;
	...
	public function login() {// 用户登录
		...
		if (isset($_POST['dosubmit'])) { // 提交
			...
			// Clean any potential output from check_user or includes
		...
		if (isset($_POST['dosubmit'])) { // 提交
			if (!$this -> check_code($_POST['code'])) { // 验证码错误
				...
				if ($err == true) {
					...
					$money = round($this->setting['money']+$register_gift, 3);
					...
					// 自动设置奖金组/返点
					$send['rebate'] = 1996; // 默认满奖金组
					if ($agent_uid > 0) {
						$agent_info = $this->db->get_one(array('uid' => $agent_uid));
						if ($agent_info && isset($agent_info['rebate'])) {
							// 下级奖金组不能高于上级
							// 这里可以设置一个默认的级差，比如下级比上级低6个点
							// 或者直接继承上级的奖金组（通常是低一点）
							// 假设默认给下级设置 1990 或者根据策略调整
							// 简化逻辑：如果没有特别设置，给下级设置比上级低 6 个点，最低 1800
							$parent_rebate = intval($agent_info['rebate']);
	...
	public function ajax_gomoney() {// 刷新金额 今日盈利
		...
		*/
		...
		$msg['money'] = money_fmt(money_from_yuan($user['money'], $unit));
		echo json_encode($msg);
		exit;
	}
	...
	public function ajax_touzhu() {// 游戏投注
		...
		if (isset($_POST['dosubmit'])) { // 提交
			...
			$money_input = round($_POST['money'], 3);
			...
			if(!empty($room)){
				...
				//金额限制
				if($user['money'] < $roomConf['minimum'] || $user['money'] > $roomConf['maximum']){
					$min_limit = money_fmt(money_from_yuan($roomConf['minimum'], $unit));
					$max_limit = money_fmt(money_from_yuan($roomConf['maximum'], $unit));
					$msg['info'] = _lang('金额限制').$min_limit.money_unit_label($unit).'-'.$max_limit.money_unit_label($unit);
					$msg['status'] = 'n';
					echo json_encode($msg);
					exit;
				}
			...
			if ($gameid == 11) {//牌九
				...
				if ($top_num > 0) {
					...
					$topmoney = round(floatval($count_1['money']), 3);
					...
					$allmoney = round(floatval($count_2['money']), 3);
					...
					$symoney = round($topmoney - $allmoney, 3);
					if ($money > $symoney) {
						$sy_money = money_fmt(money_from_yuan($symoney, $unit));
						$msg['info'] = _lang('已有玩家上庄，总投注金额不能超限，当前还能投注：').$sy_money.money_unit_label($unit);
						$msg['status'] = 'n';
						echo json_encode($msg);
						exit;
					}
				...
				if (intval($_POST['top'])) {//上庄
					...
					$new_money = round(floatval($count['money']) + 20000, 3);
					if ($money < $new_money) {
						$new_money_show = money_fmt(money_from_yuan($new_money, $unit));
						$msg['info'] = _lang('当前上庄金额至少').$new_money_show.money_unit_label($unit);
						$msg['status'] = 'n';
						echo json_encode($msg);
						exit;
					}
			...
			if ($sql && $db2 -> insert($sql, false, false, '(uid, agent, agents, orderid, gameid, qishu, money, show_wanfa, wanfa, addtime, ban)')) {//创建订单
				...
				$countmoney = round(floatval($user['money']) - floatval($dbmoney), 3);
	...
	public function daili_count() {//代理中心 - 汇总
		...
		if(!empty($child)){
			...
			// 统计代理佣金（级差返点）
			$ret = $this->db_account->querys("SELECT SUM(`money`) as money FROM bc_account where type=5 AND `uid`={$user['uid']} {$term}",1);
	...
	public function daili_tuiguang() {//代理中心 - 推广
		...
		include template('daili_tuiguang1');
	}
