...
class user extends daili {
	...
	public function edit() {
		...
		if ($data) {
			if (isset($_POST['dosubmit'])) {
				...
				// 处理返点
				$rebate = isset($_POST['rebate']) ? floatval($_POST['rebate']) : 0;
				if ($rebate > 1996 || $rebate < 0) {
					showmessage('返点设置错误！', HTTP_REFERER);
				}
				$my_info = $this->db->get_one(array('uid'=>$this->uid));
				$my_rebate = isset($my_info['rebate']) && $my_info['rebate'] > 0 ? $my_info['rebate'] : 1996;
				// 如果是修改下级，必须比自己低
				if ($rebate > $my_rebate) {
					showmessage('返点不能高于上级代理！', HTTP_REFERER);
				}
	...
	public function count_order() {
		...
		$user_info = $user_db->get_one(array('uid' => $uid), 'unit');
		$unit = $user_info && $user_info['unit'] ? $user_info['unit'] : 'yuan';
		$page = isset($_GET['page']) && intval($_GET['page']) ? intval($_GET['page']) : 1;
		$db = base::load_model('cash_model');
		$list = $db->listinfo('uid='.$uid, 'id DESC', $page, 5);
		$pages = $db->pages;
